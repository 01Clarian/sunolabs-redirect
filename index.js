...
#debug {
  margin-top: 30px; padding: 15px; background: #1a1a1a; border-radius: 8px;
  font-size: 11px; color: #888; text-align: left; max-width: 520px;
  margin-left: auto; margin-right: auto; font-family: 'Courier New', monospace;
...
<body>
  ...
  <button id="sendBtn" data-reference="${esc(reference)}">💸 Send with Wallet</button>
  <p class="info">Compatible with Phantom & Solflare</p>
...
async function sendPayment() {
  ...
  log("🖱️ Button clicked. Reference: " + ref);
  const wallet = getWallet();
  if (!wallet) {
    alert("⚠️ Please install Phantom or Solflare first.");
    return;
  }
  ...
  try {
    ...
    // ✅ Add reference for Solana Pay tracking
    if (ref) {
      ix.keys.push({
        pubkey: new PublicKey(ref),
        isSigner: false,
        isWritable: false
      });
      log("📎 Added reference key: " + ref, "success");
    }
    ...
    log("❌ Payment failed: " + err.message, "error");
